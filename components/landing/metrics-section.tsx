...
const skills = [
  { 
    ...
    sublabel: "React, Vue, Svelte, Next.js",
  },
  { 
    value: 8, 
    suffix: "", 
    prefix: "",
    label: "Backend Tech",
    sublabel: "Node.js, Python, FastAPI",
  },
  { 
    value: 5, 
    suffix: "+", 
...
export function MetricsSection() {
  ...
  return (
    <section id="skills" ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      <GridBackground />
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
            ...
            <div className="text-4xl md:text-5xl lg:text-6xl font-display tracking-tight mb-4 whitespace-nowrap overflow-hidden">
              <AnimatedNumber end={skills[0].value} suffix={skills[0].suffix} prefix={skills[0].prefix} />
            </div>
            <div className="mb-6">
              <DotGraph color="white" height={36} freq1={0.28} freq2={0.09} freqT={0.5} speed={0.018} baseline={0.35} amplitude={0.55} />
            </div>
            <div className="text-lg text-foreground mb-2">{skills[0].label}</div>
            <div className="text-sm text-muted-foreground font-mono">{skills[0].sublabel}</div>
          </div>
          ...
          {skills.slice(1).map((skill, index) => (
            ...
            >
              <div className="w-full">
                <div className="text-sm text-muted-foreground font-mono mb-2">{skill.sublabel}</div>
                <div className="text-base text-foreground mb-3">{skill.label}</div>
                <DotGraph
              ...
              <div className="text-3xl md:text-4xl lg:text-5xl font-display tracking-tight w-full">
                <AnimatedNumber end={skill.value} suffix={skill.suffix} prefix={skill.prefix} />
              </div>
            </div>
          ))}
        </div>
